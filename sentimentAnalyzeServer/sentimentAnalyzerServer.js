...
app.get("/url/sentiment", (req,res) => {
    ...
    naturalLanguageUnderstanding.analyze(analyzeParams)
        .then(analysisResults => {
            sentiments = analysisResults.result.keywords[0].sentiment;
            return res.send(sentiments, null, 2);
        })
        .catch(err => {
            return res.send("Could not do desired operation "+err);
        });
    ...
    naturalLanguageUnderstanding.analyze(analyzeParams)
        .then(analysisResults => {
            ...
            if (keywords.length > 0) {
                emotions = analysisResults.result.keywords[0].emotion;
                return res.send(emotions, null, 2);
            }
            else {
                return res.send("Try another text, could not find anything interesting for this one :(\n");
            }
        })
        .catch(err => {
            return res.send("Could not do desired operation "+err);
